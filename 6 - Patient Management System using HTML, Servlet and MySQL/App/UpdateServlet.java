...
public class UpdateServlet extends HttpServlet{
      ...
      throws ServletException, IOException {
		...
		out.print("<head><title>PMS | Update record</title></head>" );
		out.print(" <body><h1>Patient Details - Update records</h1>");
		...
		try {
			...
			// Execute SQL query
			PreparedStatement pst = conn.prepareStatement("update records set phno=? where id=?");
			pst.setInt(1, phno);
			pst.setString(2,id);
			...
			out.println("<h3> Updated phone number in database </h3></body></html>");
